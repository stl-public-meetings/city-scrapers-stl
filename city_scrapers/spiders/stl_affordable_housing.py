...
class StlAffordableHousingSpider(CityScrapersSpider):
    name = "stl_affordable_housing"
    agency = "St. Louis Affordable Housing Commission"
    ...
    def start_requests(self):
        url = (
            "https://www.stlouis-mo.gov/government/departments/affordable-housing/"
            "documents/ahc-meeting-materials.cfm"
        )
        ...
        calendar_urls = [
            (
                ...
                "past-meetings.cfm?span=-30&department=10"
            ),
    ...
    def _get_event_urls(self, response):
        ...
        for url, sponsor in zip(event_urls, event_sponsors):
            if "affordable housing" in sponsor.lower():
    ...
    def _parse_event(self, response):
        ...
        links_key = datetime.strftime(start, "%m-%Y")
    ...
    def _parse_title(self, response):
        ...
        if "canceled" in title.lower() or "cancelled" in title.lower():
            title = "Cancelled Affordable Housing Commission"
        else:
            title = "Affordable Housing Commission"
    ...
    def _parse_links(self, response):
        ...
        patterns = {"monthyyyy": [r"(?P<date>[A-Z][a-z]* \d{4})", "%B %Y"]}
        ...
        for link, description in zip(links, descriptions):
            if "agenda" in description.lower() or "materials" in description.lower():
                ...
                continue
        if dt is not None:
            formatted_date = datetime.strftime(dt, "%m-%Y")
