...
class StlZoningSpider(CityScrapersSpider):
    ...
    def start_requests(self):
        """Create agenda map and then parse meeting events"""
        urls = [
            (
        ...
        for url in urls:
            yield scrapy.Request(
                url=url, method="GET", callback=self._get_agenda_urls, dont_filter=True
            )

        calendar_urls = [
            (
                "https://www.stlouis-mo.gov/events/"
                "past-meetings.cfm?span=-30&department=294"
            ),
    ...
    def _parse_event(self, response):
        ...
        meeting["links"] = []
        if "adjustment" in meeting["title"].lower():
            if links_key in self.agenda_map_adjustment.keys():
                meeting["links"] = self.agenda_map_adjustment[links_key]
        if "conditional" in meeting["title"].lower():
            if links_key in self.agenda_map_conditional.keys():
                meeting["links"] = self.agenda_map_conditional[links_key]
