...
class StlDevelopmentCorpSpider(CityScrapersSpider):
    name = "stl_development_corp"
    agency = "St. Louis Development Corporation Board"
    ...
    def start_requests(self):
        url = (
            ...
            "documents/stlouis-development-corporation-board-documents.cfm"
        )
        ...
        calendar_urls = [
            (
                ...
                "past-meetings.cfm?span=-30&department=366"
            ),
    ...
    def _get_event_urls(self, response):
        ...
        for url, sponsor in zip(event_urls, event_sponsors):
            if "development corporation" in sponsor.lower():
    ...
    def _parse_title(self, response):
        ...
        if "development corporation" in header.lower():
            title = "St. Louis Development Corporation"

            if "cancel" in header.lower():
                title += " cancelled"

        else:
            title = header.replace("SLDC", "St. Louis Development Corporation")
    ...
    def _parse_classification(self, response):
        ...
        if "committee" in title.lower():
            return COMMITTEE
        elif "open house" in title.lower():
            return FORUM
        else:
            return BOARD
    ...
    def _parse_location(self, response):
        ...
        if "zoom" in title.lower():
            name = "Zoom"
            address = ""
        elif "teleconference" in title.lower():
            name = "Teleconference"
            address = ""
